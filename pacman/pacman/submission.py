...
class ExpectimaxAgent(MultiAgentSearchAgent):
    def getAction(self, gameState: GameState) -> str:
        move_scores = self.emoverate(gameState)
    ...
    def expectimax(self, state, depth, agent_index):
        ...
        func = max if agent_index == 0 else mean  # max for pacman, mean for ghosts
        move_results = self.emoverate(state, depth, agent_index).values()
        return func(move_results)

    def emoverate(self, state, depth=0, agent_index=0):
        def score_func(move):
            return self.expectimax(state.generateSuccessor(agent_index, move), depth, agent_index + 1)

        if agent_index == state.getNumAgents() - 1:
            return self.emoverate(state, depth+1, 0)

        return {move: score_func(move) for move in state.getLegalActions(agent_index)}
